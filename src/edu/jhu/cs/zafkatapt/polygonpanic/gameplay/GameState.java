...
public class GameState implements PanicInterface
{
	 ...
	 * 
	 * @param player1
	 *            Player 1, the player on the left
	 * @param player2
	 *            Player 2, the player on the right
	 */
	...
	public List<Unit> update()
	{
		...
		{
			List<Unit> returnList = new ArrayList<Unit>(units.get(0).size()
					+ units.get(1).size());
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < units.get(i).size(); j++)
			{
				...
				if (unit.isAttacking())
				{
					// is the unit in range of the enemy base and the enemy has
					// no units
					if (units.get(enemy).isEmpty()
							&& isUnitInBaseAttackRange(unit, 0))
					{
						unit.attack(players.get(enemy).getMyBase(), delta);

						if (players.get(enemy).getMyBase().getCurrHealth() <= 0)
						{
							fireEvent(new PanicEvent(false, false, false,
									player + 1));
						}
					...
					if (!units.get(enemy).isEmpty()
							&& isUnitInFrontLineAttackRange(unit, 0))
					{
						...
						{
							if (frontLines.get(enemy).size() == 1
									&& !isUnitInBaseAttackRange(unit, 0))
				...
				{
					if (player == 0)
						unit.setPosition(frontLines.get(enemy).peek()
								.getPosition()
								- ATTACK_RANGE);
					else
						unit.setPosition(frontLines.get(enemy).peek()
				...
				/*
				 * //unit is not currently in the front line but is at the
				 * proper position if (!frontLines.get(player).contains(unit) &&
				 * unit.getPosition() ==
				 * frontLines.get(player).peek().getPosition()) {
				 * frontLines.get(player).add(unit); }
				 * 
				 * //front line queue will be cleared and unit will be added
				 * else if ((player == 0 && unit.getPosition() >
				 * frontLines.get(player).peek().getPosition()) || (player == 1
				 * && unit.getPosition() <
				 * frontLines.get(player).peek().getPosition())) {
	...
	{
		return players.get(0).getMyBase().getCurrHealth();
	}

	@Override
	public double getBlueBaseHealth()
	{
		return players.get(1).getMyBase().getCurrHealth();
	}
	...
	public double[] getFrontLineData()
	{
		...
		{
			data[FrontLineData.p1Position] = -1;
			data[FrontLineData.p1Health] = players.get(0).getMyBase().getCurrHealth();
		} else
		{
			data[FrontLineData.p1Position] = frontLines.get(0).peek().getPosition();
			data[FrontLineData.p1Health] = frontLines.get(0).peek().getCurrHealth();
		}

		if (frontLines.get(1).isEmpty())
		{
			data[FrontLineData.p2Position] = -1;
			data[FrontLineData.p2Health] = players.get(1).getMyBase().getCurrHealth();
		} else
		{
			data[FrontLineData.p2Position] = frontLines.get(1).peek().getPosition();
			data[FrontLineData.p2Health] = frontLines.get(1).peek().getCurrHealth();
		}
	...
	{
		return frontLines.get(0);
	}

	@Override
	public Queue<Unit> getP2FrontLine()
	{
		return frontLines.get(1);
	}

	@Override
	public void setP1FrontLine(Queue<Unit> p1FrontLine)
	{
		frontLines.set(0, (LinkedList<Unit>)(p1FrontLine));
	}

	@Override
	public void setP2FrontLine(Queue<Unit> p2FrontLine)
	{
		frontLines.set(1, (LinkedList<Unit>)(p2FrontLine));
	}

	/**
	 * Combines the units of each player and returns one list. Removes units
	 * that have been killed, and fires the appropriate event if no attacks are
	 * occurring.
	 * 
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < units.get(i).size(); j++)
			{
				...
				if (!unit.isAlive())
				{
					...
					players.get((i == 0) ? 1 : 0).addGold(
							unit.getGoldValue()
									* players.get((i == 0) ? 1 : 0).getMyBase()
											.getGoldRate());
					...
					/*
					 * if (frontLines.get(i).isEmpty()) updateFrontLine(i);
					 */
	...
	/**
	 * Checks if the sepcified unit is within attack range of the enemy's front
	 * line
	 * 
	 * @param unit
	 *            the unit
	 * @param delta
	 *            the time since the last update (used to calculate move)
	 * @return <code>true</code> if the unit is within attack range of the
	 *         enemy's front line and <code>false</code> otherwise
	 */
	private boolean isUnitInFrontLineAttackRange(Unit unit, long delta)
	{
		if (unit.getDirection() == 1
				&& frontLines.get(1).peek() != null
				&& unit.getMovementPosition(delta) >= (frontLines.get(1).peek()
						.getPosition() - ATTACK_RANGE))
		{
			return true;
		}
		...
		if (unit.getDirection() == -1
				...
				&& unit.getMovementPosition(delta) <= (frontLines.get(0).peek()
						.getPosition() + ATTACK_RANGE))
		{
			return true;
		}
	 ...
	 *            the time since the last update (used to calculate move)
	 * @return <code>true</code> if the unit is within attack range of the
	 *         enemy's base and <code>false</code> otherwise
	 */
	private boolean isUnitInBaseAttackRange(Unit unit, long delta)
	{
		if (unit.getDirection() == 1
				&& unit.getMovementPosition(delta) >= P2_BASE_LOCATION
						- ATTACK_RANGE)
		{
			return true;
		}
		...
		if (unit.getDirection() == -1
				&& unit.getMovementPosition(delta) <= P1_BASE_LOCATION
						+ ATTACK_RANGE)
		{
			return true;
		}
	 ...
	 * @param player
	 *            the player casting
	 * @return <code>true</code> if the cast was successful and
	...
	private boolean castGravWell(int player)
	{
		...
		if ((time - lastGravWell) >= Spell.getCoolDown(Spell.GRAV_WELL)
				&& getPlayerEnergy(player) >= Spell
						.getEnergyCost(Spell.GRAV_WELL))
		{
			lastGravWell = time;

			players.get(player).getMyBase().spendEnergy(
					Spell.getEnergyCost(Spell.GRAV_WELL));
	...
	private boolean castHorror(int player)
	{
				...
				&& getPlayerEnergy(player) >= Spell.getEnergyCost(Spell.HORROR))
		{
			...
			players.get(player - 1).getMyBase().spendEnergy(
					Spell.getEnergyCost(Spell.HORROR));
	...
	private boolean castQuake(int player)
	{
				...
				&& getPlayerEnergy(player) >= Spell.getEnergyCost(Spell.QUAKE))
		{
			...
			players.get(player - 1).getMyBase().spendEnergy(
					Spell.getEnergyCost(Spell.QUAKE));
